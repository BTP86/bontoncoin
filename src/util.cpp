...
const char * const BITCOIN_CONF_FILENAME = "bontoncoin.conf";
const char * const BITCOIN_PID_FILENAME = "bontoncoin.pid";
...
#else
    const char* pszModule = "bontoncoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "bontoncoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/bontoncoin";
#else
    // Unix
    return pathRet / ".bontoncoin";
#endif
#endif
}
